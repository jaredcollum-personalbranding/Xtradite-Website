# Xtradite Digital â€” website frontend

Plain static HTML/CSS/JS. No build step, no framework, no npm install. Every page is wired
live to a Supabase project (Postgres + auto-generated REST API) â€” content you edit in the
Supabase Table Editor appears here automatically, no code changes needed.

## How it's wired

- `assets/js/supabase-client.js` â€” creates the Supabase client using this project's public
  URL and anon (publishable) key. Safe to keep hardcoded: Row Level Security policies (see
  `supabase/schema.sql`) restrict what the anon key can actually do â€” public read on content
  tables, insert-only on `contact_submissions`.
- `assets/js/cms.js` / `blog.js` â€” read helpers for the `services`, `industries`,
  `case_studies` and `blog_posts` tables.
- `assets/js/forms.js` â€” inserts Contact page submissions into `contact_submissions`.
- `assets/js/pages/*.js` â€” one script per page, fetches and renders that page's data.
- `../api/location.js` â€” renders the `/locations` and `/uk/...` hierarchy from the
...
`slug` query param and fetch that one row â€” add/remove/edit rows in the Supabase Table Editor
...
not in this code â€” edit rows directly in the Supabase dashboard's Table Editor for this
...
by design â€” RLS grants it insert-only).
...
and the content migrated from the site's original Wix CMS/Blog) â€” only needs running once
against a fresh Supabase project. `supabase/migrations/` holds later schema changes, applied
in order â€” e.g. `002_blog_tags_seo.sql` adds `blog_posts.tags` (a `text[]`, used for the
tag filter on the Insights page â€” just add tag strings to a post's `tags` array in the
...
Any static host works â€” this is just files. Drag-and-drop the whole `frontend/` folder onto
...
path â€” the page scripts use ES modules, which browsers block from `file://` for security
reasons.
...
- **Google Analytics (GA4)** â€” the `gtag.js` snippet (measurement ID `G-WYXTKGJ9JS`) is
...
- **Server-side lead tracking** â€” `api/track-lead.js` (Vercel serverless function) sends
  ...
  variable in the Vercel project settings** (the GA4 Measurement Protocol API secret) â€”
  without it the function no-ops silently.
- **AdSense** â€” the `adsbygoogle.js` loader (`ca-pub-7308041122340160`) is in every
  page's `<head>`; `ads.txt` at the site root declares the same publisher ID as Google
  requires.
- **Sitemap** â€” `https://www.xtradite-digital.co.uk/sitemap.xml` is served by
  ...
  values for geographic routes â€” nothing to regenerate.
- **robots.txt / llms.txt** â€” `robots.txt` allows all crawlers (including named AI/LLM
...
- **LinkedIn link** â€” the footer's LinkedIn icon points to `#`. Update it in the footer
  markup (search `aria-label="LinkedIn"`) once you have the real company URL.
- **Testimonials** â€” the 3 testimonials on Home and About, and the 1 quote on the
...
- **Legal pages** â€” `legal/privacy.html`, `legal/terms.html`, `legal/cookies.html` are
  lightweight drafted templates, not lawyer-reviewed. Have them checked before launch.
- **Photography** â€” case study card images are currently a placeholder gradient block.
