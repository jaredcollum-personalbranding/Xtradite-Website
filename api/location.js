...
function serviceLinks(location, locationServices) {
  return locationServices.map((item) => `<a class="location-link" href="${pathFor(location, item.slug)}"><span>${esc(item.title)}<small>${esc(item.searchLabel)} for ${esc(location.name)}</small></span><span>→</span></a>`).join("");
}
